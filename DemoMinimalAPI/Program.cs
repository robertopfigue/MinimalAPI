...
void MapActions(WebApplication app)
{
    ...
    ) =>
    {
        if (registerUser == null)
            return Results.BadRequest("Usuário não infomrado");
    ...
    app.MapPut("/fornecedor/{id}", [Authorize] async (
        ...
        Fornecedor fornecedor) =>
    {
        var forncedorBanco = await context.Fornecedores.AsNoTracking<Fornecedor>()
    ...
    app.MapDelete("/fornecedor/{id}", [Authorize] async (
        ...
        MinimalContextDb context) =>
    {
        var forncedorBanco = await context.Fornecedores.FindAsync(id);
        if (forncedorBanco == null) return Results.NotFound();

        context.Fornecedores.Remove(forncedorBanco);
        var result = await context.SaveChangesAsync();

        return result > 0
            ? Results.NoContent()
            : Results.BadRequest("Houve um erro ao deletar o registro");
